...
define('FLASH_CLIENT_URL', "https://images.habbo.com/gordon/PRODUCTION-201711211204-412329988/");
...
function ExtractCastOffset($xml, $assets, $parent)
{
	$ret = array();
	foreach($xml->library->assets->asset as $asset)
	{
		$name = (string)$asset->attributes()->name;
		//consoleLog($xml->library["name"]."_".$name);
		foreach($asset->param as $param)
		{
			if($param->attributes()->key == 'offset')
			{
				$offset = explode(",", (string)$param->attributes()->value);
				//consoleLog($xml->library["name"]."_".$name . " -- " . (in_array($xml->library["name"]."_".$name, $assets) ? "true" : "false"));
				$ret[$name] = array('x' => $offset[0], 'y' => $offset[1], 'flipped' => !(in_array($parent."_".$name, $assets)));
				break;
			}
		}
	}
	...
	foreach ($xmls as $symbol_id => $xml) {
		...
		if($_xml !== false) {
			...
			if(($_name == "animationSet" || $_name == "geometry" || $_name == "partSets" || $_name == "actions"/* || $_name == "figuredata" || $_name == "offsets"*/) && $swfID == "Habbo") {
				...
				ExtractCastOffset($_xml, $assets, $swfID);
			}
...
ExtractFigureData("http://localhost/figuredata.xml");
ExtractItemSet();
